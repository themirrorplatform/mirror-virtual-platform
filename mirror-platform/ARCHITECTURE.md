...
**PR1 — The source materials. COMPLETE (as of 2026-06-18) — every batch is now
spec-unblocked.** §0 rule 8 (don't invent UX) is satisfiable end-to-end.

*Received & vendored in `_source/`:* the **Everything Spec** (master bible) · Build
Plan · **Composition Schema** · **Lexicon Schema** · **Voice Spec + Surface-Language
Sweep** · **Telemetry Spec** · **Contribution-Conduct Spec** · **SEO-Shareability
Spec** · Data-Permanence Spec · Device-Access Spec · `protocol.py` (the engine) ·
`MirrorPlatform.jsx` (the §0.8 prototype) · the Attempt Register.

*Per-batch coverage — all green:*
- **Batch 1 (P1–P4):** Everything §0/§4/§5/§16, Composition, Device, Voice → ready.
- **Batch 2 (P5 Composition / P6 Lexicon):** both specs in hand → ready.
- **Batch 4 (P14):** Telemetry Spec → ready.
- **Batches 6–7 (P10–P13):** Contribution-Conduct + the two reference prototypes
  (admin console + two-face Reader/Admin with tiers, paywall, encounters band) → ready.
- **Batch 8 (P15):** SEO Spec → ready.

*One non-spec item still wanted (not blocking until Batch 9):* the **canonical
`graph.json`** — the `protocol.py --seed` output (~358 nodes) for **P17**. The
prototype embeds a slim subset (vendored at `_source/graph.slim.json`); the
authoritative seed is generated by `protocol.py` from the register `.md` files.
Drop the generated `graph.json` (+ register sources) when convenient.

**Materials are effectively complete. The remaining gates to start building are
PR2 (Supabase access) and your go-ahead — not more specs.**
