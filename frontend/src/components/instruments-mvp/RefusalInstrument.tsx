...
import { Ban, FileText, Shield, ArrowRight, AlertCircle } from 'lucide-react';
...
interface RefusalInstrumentProps {
  ...
  onViewConstitution: (articleId: string) => void;
  onViewSpeechContract: () => void;
  onClose: () => void;
}
    ...
    >
      ...
      <div className="relative w-full max-w-3xl bg-[var(--color-surface-card)] border border-[var(--color-border-subtle)] rounded-2xl shadow-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="p-6 border-b border-[var(--color-border-subtle)] flex-shrink-0">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h2 className="mb-1">Boundary Reached</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="p-4 rounded-xl bg-[var(--color-surface-card)] border border-[var(--color-border-subtle)]">
              ...
              <div className="capitalize">{refusal.layer}</div>
            </div>
            <div className="p-4 rounded-xl bg-[var(--color-surface-card)] border border-[var(--color-border-subtle)]">
              <div className="text-[var(--color-text-muted)] mb-2">Constitution</div>
              <div>{refusal.constitution}</div>
            </div>
          </div>
          ...
          {refusal.allowedReframes.length > 0 && (
            <div>
              <h3 className="mb-3">What is allowed</h3>
              <div className="space-y-2">
            ...
            <div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm mb-1">View Speech Contract</div>
                    <div className="text-xs text-[var(--color-text-muted)]">
        ...
        <div className="p-6 border-t border-[var(--color-border-subtle)] flex-shrink-0">
          <button
            onClick={onClose}
            className="w-full px-4 py-2 rounded-xl bg-[var(--color-accent-gold)] text-[var(--color-text-inverse)] transition-all"
          >
            Return to Field
          </button>
        </div>
...
export const EXAMPLE_REFUSALS: RefusalContext[] = [
  ...
  {
    refusalText: 'I cannot assess whether you should "fix" this aspect of yourself. That framing assumes something is broken, which imposes a value I cannot hold.',
    layer: 'commons',
    constitution: 'Core Constitution v1.0',
    invariantClass: 'manipulation',
    allowedReframes: [
      'I can show how you\'ve talked about this over time',
      'I can show where this pattern appears in other reflections',
      'I can show the tensions you\'ve named around this',
    ],
    speechContractLink: true,
    relatedArticles: [
      {
        id: 'art-2',
        title: 'Article II: Reflection',
        excerpt: 'Reflection without judgment or agenda. Only mirroring what appears.',
      },
    ],
  },
];