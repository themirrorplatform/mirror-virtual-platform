 ...
 * Constitutional: Appears only when user needs to select layer
 * Enhanced for Next.js with TypeScript improvements
 */

import { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Shield, Globe, Wrench, FileText, AlertCircle, X } from 'lucide-react';
import type { Layer } from '../hooks/useMirrorState';
...
export function EntryInstrument({ onComplete, onDismiss, trigger }: EntryInstrumentProps) {
  const [selectedLayer, setSelectedLayer] = useState<Layer>('sovereign');
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 px-4 py-2 rounded-xl bg-[var(--color-accent-gold)] text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
            >
              Acknowledge
            </button>
          </div>
    ...
    <InstrumentPanel title="How this reflection exists" onClose={onDismiss}>
      <div className="space-y-8">
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="mb-1 text-[var(--color-text-primary)]">{option.title}</h4>
        ...
        {selectedLayer !== 'sovereign' && (
          <div className="p-4 rounded-xl bg-[var(--color-accent-gold)]/5 border border-[var(--color-accent-gold)]/20">
            <div className="flex items-start gap-2 mb-2">
              ...
              <h5 className="text-sm text-[var(--color-text-primary)]">What changes</h5>
            </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => setViewingLicenses(true)}
            className="flex-1 px-4 py-2 rounded-xl bg-[var(--color-accent-gold)] text-white transition-all hover:opacity-90"
          >
            Proceed
          </button>
        </div>
